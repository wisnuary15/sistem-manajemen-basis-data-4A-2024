CREATE DATABASE tugaspraktikumm3;
USE tugaspraktikumm3;
...
-- no1
DELIMITER //
CREATE PROCEDURE get_anggota (IN kode INT(100))
BEGIN
	SELECT * FROM anggota WHERE id_anggota = kode;
END //
DELIMITER ;

CALL get_anggota(101);

DELIMITER //
CREATE PROCEDURE get_pengembalian (IN kode INT(100))
BEGIN
	SELECT * FROM pengembalian WHERE kode_kembali = kode;
END //
DELIMITER ;

CALL get_pengembalian(401);

-- no 2

DELIMITER //
CREATE PROCEDURE get_buku1(IN kode INT (10))
BEGIN
...
DELIMITER //
CREATE PROCEDURE get_petugas1 (IN kode INT (10), IN namapet VARCHAR (25))
BEGIN 
	SELECT * FROM petugas WHERE id_petugas = kode AND nama = namapet; 
END //
DELIMITER ;

CALL get_petugas1(202, 'wahyu');

-- no 4

DELIMITER //
CREATE PROCEDURE get_petugas2 (
	IN kode INT (10),
	IN namapet VARCHAR (25),
	IN usernm VARCHAR (15))
BEGIN
	SELECT * FROM petugas WHERE id_petugas = kode 
	AND nama = namapet
	AND username = usernm;
END //
DELIMITER;

CALL get_petugas2(203, 'dinda', 'dinda5');
...
CALL insert_petugas (225, 'sri05', 'sr115', 'Sriati');

SELECT * FROM petugas;

-- no 6
DELIMITER //
CREATE PROCEDURE JumlahBuku(OUT jmlBuku INT(10))
	BEGIN 
		SELECT COUNT(jumlah_buku) INTO jmlBuku FROM buku;
	END //
DELIMITER ;
CALL JumlahBuku(@jmlBuku);
SELECT @jmlBuku;
...
BEGIN
  ...
  FROM buku
  WHERE pengarang_buku = namapeng AND tahun_buku = tahun;
...
SET @tahun = '2009';
CALL jumlahBuku ('Ahmad Fuadi', @tahun , @total);
